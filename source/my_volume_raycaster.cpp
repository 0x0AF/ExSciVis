...
float g_sampling_distance_fact = 4.0f;
...
glm::ivec2 g_window_res = glm::ivec2(1366, 768);
...
struct Manipulator
{
    Manipulator() : m_turntable(), m_mouse_button_pressed(0, 0, 0), m_mouse(0.0f, 0.0f), m_lastMouse(0.0f, 0.0f) {}
    glm::mat4 matrix()
    {
...
bool read_volume(std::string &volume_string)
{
    ...
    unsigned max_dim = std::max(std::max(g_vol_dimensions.x, g_vol_dimensions.y), g_vol_dimensions.z);
...
void UpdateImGui()
{
    ...
    io.MouseDown[0] = mousePressed[0] ||
                      glfwGetMouseButton(g_win.getGLFWwindow(), GLFW_MOUSE_BUTTON_LEFT) !=
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        g_volume_program = loadShaders(g_file_vertex_shader, g_file_fragment_shader, g_task_chosen, g_lighting_toggle, g_shadow_toggle, g_opacity_correction_toggle);
    }
